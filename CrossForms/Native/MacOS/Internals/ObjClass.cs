...
internal class ObjClass: NativeManaged<IntPtr> {
	...
	public IntPtr AddMethod<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] D> (string name, D fn) where D: Delegate {
		...
		AddClassMethod(inner, sel, Marshal.GetFunctionPointerForDelegate<D>(fn), types.ToString());
		return sel;
	}
	public IntPtr AddMethod<D> (string name, D fn, string types) where D: Delegate {
		var sel = ObjSelector.Register(name);
		AddClassMethod(inner, sel, Marshal.GetFunctionPointerForDelegate<D>(fn), types);
		return sel;
	}
	...
	public IntPtr ReplaceMethod<D> (string name, D fn, string types) where D: Delegate {
		var sel = ObjSelector.Register(name);
		ReplaceClassMethod(inner, sel, Marshal.GetFunctionPointerForDelegate<D>(fn), types);
		return sel;
	}
